        ...
        for file in files:
            if file.endswith(".md") and file != "index.md":
                ...
                with open(os.path.join(subdir, file), 'r', encoding='utf-8') as md_file:
                    ...
                    if img_match:
                        relative_img_path = img_match.group(1)
                        # Calculate the correct image path relative to the HTML file
                        img_src = os.path.relpath(os.path.join(subdir, relative_img_path), root_dir).replace("\\", "/")
                    else:
